...
void
TimeSimpleTestCase::DoTimeOperations()
{
    // Test Multiplication
...
{
    NS_TEST_ASSERT_MSG_EQ_TOL(Years(1.0).GetYears(), 1.0, Years(1).GetYears(), "is 1 really 1 ?");
    NS_TEST_ASSERT_MSG_EQ_TOL(Years(10.0).GetYears(),
                              10.0,
                              Years(1).GetYears(),
                              "is 10 really 10 ?");
    NS_TEST_ASSERT_MSG_EQ_TOL(Days(1.0).GetDays(), 1.0, Days(1).GetDays(), "is 1 really 1 ?");
    NS_TEST_ASSERT_MSG_EQ_TOL(Days(10.0).GetDays(), 10.0, Days(1).GetDays(), "is 10 really 10 ?");
    NS_TEST_ASSERT_MSG_EQ_TOL(Hours(1.0).GetHours(), 1.0, Hours(1).GetHours(), "is 1 really 1 ?");
    NS_TEST_ASSERT_MSG_EQ_TOL(Hours(10.0).GetHours(),
                              10.0,
                              Hours(1).GetHours(),
                              "is 10 really 10 ?");
    NS_TEST_ASSERT_MSG_EQ_TOL(Minutes(1.0).GetMinutes(),
                              1.0,
                              Minutes(1).GetMinutes(),
                              "is 1 really 1 ?");
    NS_TEST_ASSERT_MSG_EQ_TOL(Minutes(10.0).GetMinutes(),
                              10.0,
                              Minutes(1).GetMinutes(),
                              "is 10 really 10 ?");
    ...
    NS_TEST_ASSERT_MSG_EQ_TOL(Seconds(10.0).GetSeconds(),
                              ...
                              TimeStep(1).GetSeconds(),
                              "is 10 really 10 ?");
    NS_TEST_ASSERT_MSG_EQ(MilliSeconds(1).GetMilliSeconds(), 1, "is 1ms really 1ms ?");
    NS_TEST_ASSERT_MSG_EQ(MicroSeconds(1).GetMicroSeconds(), 1, "is 1us really 1us ?");
...
template <typename T>
void
TimeSimpleTestCase::TestMultiplication(Time t, Time expected, T val, const std::string& msg)

{
    using TestEntry = std::tuple<Time, std::string>;
    std::array<TestEntry, 2> TESTS{std::make_tuple(t * val, "Test Time * value: "),
...
TimeSimpleTestCase::TestMultiplicationByIntegerTypes()
{
    ...
    TestMultiplication(t, expected, static_cast<long long>(scale), "Multiplication by long long");
    TestMultiplication(t,
                       expected,
                       static_cast<unsigned long long>(scale),
                       "Multiplication by unsigned long long");
    TestMultiplication(t, expected, static_cast<std::size_t>(scale), "Multiplication by size_t");
...
TimeSimpleTestCase::TestMultiplicationByDecimalTypes()
{
    ...
    TestMultiplication(t, expected, static_cast<double>(scale), "Multiplication by double");
}
...
TimeSimpleTestCase::TestDivisionByIntegerTypes()
{
    ...
    TestDivision(t, expected, static_cast<unsigned char>(scale), "Division by unsigned char");
    TestDivision(t, expected, static_cast<short>(scale), "Division by short");
    TestDivision(t, expected, static_cast<unsigned short>(scale), "Division by unsigned short");
    TestDivision(t, expected, static_cast<int>(scale), "Division by int");
    TestDivision(t, expected, static_cast<unsigned int>(scale), "Division by unsigned int");
    TestDivision(t, expected, static_cast<long>(scale), "Division by long");
    TestDivision(t, expected, static_cast<unsigned long>(scale), "Division by unsigned long");
    TestDivision(t, expected, static_cast<long long>(scale), "Division by long long");
    TestDivision(t,
                 expected,
                 static_cast<unsigned long long>(scale),
                 "Division by unsigned long long");
    TestDivision(t, expected, static_cast<std::size_t>(scale), "Division by size_t");
...
TimeInputOutputTestCase::CheckAs(const Time t, const std::string expect)
{
    ...
    std::cout << GetParent()->GetName() << " InputOutput:As: " << (pass ? "pass " : "FAIL ") << "\""
              << expect << "\"";
    if (!pass)
    {
        std::cout << ", got " << str;
...
TimeInputOutputTestCase::DoRun()
{
    ...
    std::cout << GetParent()->GetName() << " InputOutput: " << std::fixed << std::setprecision(9)
              << "example: in s:  " << t.As(Time::S) << std::endl;

    std::cout << GetParent()->GetName() << " InputOutput: " << std::setprecision(6)
              << "example: in ms: " << t.As(Time::MS) << std::endl;

    std::cout << GetParent()->GetName() << " InputOutput: "
              << "example: Get ns: " << t.GetNanoSeconds() << std::endl;
