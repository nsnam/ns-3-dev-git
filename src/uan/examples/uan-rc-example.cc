...
Experiment::CreateMode(uint32_t kass, uint32_t fc, bool upperblock, std::string name) const
{
    ...
    UanTxMode mode;
    mode = UanTxModeFactory::CreateMode(UanTxMode::OTHER, rate, phyrate, fcmode, bw, 2, buf.str());
    return mode;
}
...
Experiment::Run(uint32_t param)
{
    ...
    uan.SetPhy("ns3::UanPhyDual",
               "SupportedModesPhy1",
               UanModesListValue(m_dataModes),
               "SupportedModesPhy2",
               UanModesListValue(m_controlModes));

    uan.SetMac("ns3::UanMacRcGw",
               "NumberOfRates",
               UintegerValue(m_numRates),
               "NumberOfNodes",
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("SimMin",
                 "Minimum parameter to test (nodes if DoNode=1, \"a\" param otherwise)",
                 exp.m_simMin);
    cmd.AddValue("SimMax",
                 "Maximum parameter to test (nodes if DoNode=1, \"a\" param otherwise)",
                 exp.m_simMax);
