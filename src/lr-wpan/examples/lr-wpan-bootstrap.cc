 ...
 * Nodes start scanning channels 11-14 looking for beacons for a defined duration (PASSIVE SCAN). The scanning
 * start time is slightly different for each node to avoid a storm of association requests. When a node scan is completed,
 * an association request is send to one coordinator based on the LQI results of the scan. A node may not find any beacons if
 * the coordinator is outside its communication range. An association request may not be send if LQI is too low for an association.
 *
 * At the end of the simulation, an animation is generated (lrwpan-bootstrap.xml), showing the results
 * of the association with each coordinator. This simulation can take a few seconds to complete.
 */
      ...
      if (params.m_panDescList.size () > 0)
          ...
          if (params.m_panDescList [panDescIndex].m_superframeSpec.IsAssocPermit ())
            {
              std::cout << Simulator::Now ().As (Time::S)
                        ...
                        << " [" << device->GetMac ()->GetShortAddress () << " | " << device->GetMac ()->GetExtendedAddress () << "]"
              ...
              if (params.m_panDescList [panDescIndex].m_linkQuality  >= 127)
                {
                  ...
                  if (params.m_panDescList[panDescIndex].m_coorAddrMode == LrWpanAddressMode::SHORT_ADDR)
                    {
                      assocParams.m_coordAddrMode = LrWpanAddressMode::SHORT_ADDR;
                      assocParams.m_coordShortAddr = params.m_panDescList [panDescIndex].m_coorShortAddr;
                    }
                  else if (assocParams.m_coordAddrMode == LrWpanAddressMode::EXT_ADDR)
                    {
                      assocParams.m_coordAddrMode = LrWpanAddressMode::EXT_ADDR;
                      assocParams.m_coordExtAddr = params.m_panDescList [panDescIndex].m_coorExtAddr;
                      assocParams.m_coordShortAddr = Mac16Address ("ff:fe");
                    }

                  Simulator::ScheduleNow (&LrWpanMac::MlmeAssociateRequest, device->GetMac (), assocParams);
                }
              else
                {
                  std::cout << Simulator::Now ().As (Time::S)
                            << " Node " << device->GetNode ()->GetId ()
                            << " [" << device->GetMac ()->GetShortAddress () << " | " << device->GetMac ()->GetExtendedAddress () << "]"
                            << " MLME-scan.confirm: Beacon found but link quality too low for association.\n";
                }
            }
        }
      else
        {
          std::cout << Simulator::Now ().As (Time::S)
                    ...
                    << device->GetMac ()->GetExtendedAddress () << "] MLME-scan.confirm: Beacon not found.\n";
        }
    }
  else
    {
      std::cout << Simulator::Now ().As (Time::S)
                << " [" << device->GetMac ()->GetShortAddress () << " | "
                << device->GetMac ()->GetExtendedAddress () << "]  error occurred, scan failed.\n";
    }
}
...
static void AssociateIndication (Ptr<LrWpanNetDevice> device, MlmeAssociateIndicationParams params)
{
  ...
  //    a) Whether or not the device was previously associated with this PAN (the coordinator keeps a list).
  ...
  // In this example, the coordinator accepts every association request and have no association limits.
  // Furthermore, previous associated devices are not checked.

  // When short address allocation is on (set initially in the association request), the coordinator is supposed to assign a short address.
  // In here, we just do a dummy address assign. The assigned short address is just a truncated version of the device existing
  ...
  switch (params.m_status)
    {
      case LrWpanMlmeCommStatus::MLMECOMMSTATUS_TRANSACTION_EXPIRED:
        std::cout << Simulator::Now ().As (Time::S)
                  ...
                  << params.m_dstExtAddr << " EXPIRED in pending transaction list\n";
        break;
      case LrWpanMlmeCommStatus::MLMECOMMSTATUS_NO_ACK:
        std::cout << Simulator::Now ().As (Time::S)
                  ...
                  << params.m_dstExtAddr << " device registered in the pending transaction list\n";
        break;
  ...
  for (NodeContainer::Iterator i = nodes.Begin (); i != nodes.End (); i++)
    {
      ...
      Ptr<NetDevice> netDevice = node->GetDevice (0);
      Ptr<LrWpanNetDevice> lrwpanDevice = DynamicCast<LrWpanNetDevice> (netDevice);
      lrwpanDevice->GetMac ()->SetMlmeScanConfirmCallback (MakeBoundCallback (&ScanConfirm,lrwpanDevice));
      lrwpanDevice->GetMac ()->SetMlmeAssociateConfirmCallback (MakeBoundCallback (&AssociateConfirm,lrwpanDevice));
      lrwpanDevice->GetMac ()->SetMlmePollConfirmCallback (MakeBoundCallback (&PollConfirm,lrwpanDevice));
  ...
  for (NodeContainer::Iterator i = coordinators.Begin (); i != coordinators.End (); i++)
    {
      ...
      Ptr<LrWpanNetDevice> lrwpanDevice = DynamicCast<LrWpanNetDevice> (netDevice);
      lrwpanDevice->GetMac ()->SetMlmeAssociateIndicationCallback (MakeBoundCallback (&AssociateIndication,lrwpanDevice));
      lrwpanDevice->GetMac ()->SetMlmeCommStatusIndicationCallback (MakeBoundCallback (&CommStatusIndication,lrwpanDevice));
    }
