...
WriteModeCreateTestCase::DoRun()
{
    ...
    //
    f.Open(m_testFilename, std::ios::out);
    NS_TEST_ASSERT_MSG_EQ(f.Fail(),
                          false,
                          "Open (" << m_testFilename << ", \"std::ios::out\") returns error");
    ...
    NS_TEST_ASSERT_MSG_EQ(f.Fail(),
                          false,
                          "Write (write-only-file " << m_testFilename << ") returns error");
}
...
ReadModeCreateTestCase::DoRun()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(f.Fail(),
                          true,
                          "Write (read-only-file " << m_testFilename << ") does not return error");
    f.Close();
    f.Clear();
}
...
FileHeaderTestCase::DoRun()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(f.Fail(),
                          ...
                          "Open (existing-initialized-file "
                              << m_testFilename << ", \"std::ios::in\") returns error");
...
RecordHeaderTestCase::DoRun()
{
    ...
    for (uint32_t i = 0; i < 43; ++i)
    {
        NS_TEST_ASSERT_MSG_EQ(bufferIn[i],
                              bufferOut[i],
...
ReadFileTestCase::DoRun()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(f.Fail(),
                          false,
                          "Open (" << filename << ", \"std::ios::in\") returns error");
...
DiffTestCase::DoRun()
{
    ...
    //
    // Create different PCAP file (with the same timestamps, but different packets) and check that
    ...
    NS_TEST_ASSERT_MSG_EQ(f.Fail(),
                          false,
                          "Open (" << filename2 << ", \"std::ios::out\") returns error");
