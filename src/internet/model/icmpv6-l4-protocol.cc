...
void Icmpv6L4Protocol::DoDAD (Ipv6Address target, Ptr<Ipv6Interface> interface)
{
  ...
  NdiscCache::PacketWithHeader p = ForgeNS ("::",Ipv6Address::MakeSolicitedAddress (target), target, interface->GetDevice ()->GetAddress ());
...
void Icmpv6L4Protocol::ReceiveLLA (Icmpv6OptionLinkLayerAddress lla, Ipv6Address const &src, Ipv6Address const &dst, Ptr<Ipv6Interface> interface)
{
  ...
  if (!entry)
    ...
    {
      std::list<NdiscCache::PacketWithHeader> waiting;
      if (entry->IsIncomplete ())
        {
          ...
          for (std::list<NdiscCache::PacketWithHeader>::const_iterator it = waiting.begin (); it != waiting.end (); it++)
            {
...
void Icmpv6L4Protocol::HandleNS (Ptr<Packet> packet, Ipv6Address const &src, Ipv6Address const &dst, Ptr<Ipv6Interface> interface)
{
  ...
  NdiscCache::PacketWithHeader p = ForgeNA (target.IsLinkLocal () ? interface->GetLinkLocalAddress ().GetAddress () : ifaddr.GetAddress (), src.IsAny () ? Ipv6Address::GetAllNodesMulticast () : src, &hardwareAddress, flags );
  interface->Send (p.first, p.second, src.IsAny () ? Ipv6Address::GetAllNodesMulticast () : src);

  /* not a NS for us discard it */
}

NdiscCache::PacketWithHeader Icmpv6L4Protocol::ForgeRS (Ipv6Address src, Ipv6Address dst, Address hardwareAddress)
{
  ...
  return NdiscCache::PacketWithHeader (p, ipHeader);
}

NdiscCache::PacketWithHeader Icmpv6L4Protocol::ForgeEchoRequest (Ipv6Address src, Ipv6Address dst, uint16_t id, uint16_t seq, Ptr<Packet> data)
{
...
NdiscCache::PacketWithHeader Icmpv6L4Protocol::ForgeNA (Ipv6Address src, Ipv6Address dst, Address* hardwareAddress, uint8_t flags)
{
...
NdiscCache::PacketWithHeader Icmpv6L4Protocol::ForgeNS (Ipv6Address src, Ipv6Address dst, Ipv6Address target, Address hardwareAddress)
{
  NS_LOG_FUNCTION (this << src << dst << target << hardwareAddress);
  Ptr<Packet> p = Create<Packet> ();
  Ipv6Header ipHeader;
  ...
  if (entry)
    {
      if (entry->IsReachable () || entry->IsDelay ())
        ...
        {
          /* queue packet */
          entry->AddWaitingPacket (NdiscCache::PacketWithHeader (p, ipHeader));
          return false;
        }
    }
  else
    {
      ...
      entry->MarkIncomplete (NdiscCache::PacketWithHeader (p, ipHeader));
      entry->SetRouter (false);
