...
void
ArpCache::DoFlush(bool force)
{
    NS_LOG_FUNCTION(this << force);
    for (auto it = m_arpCache.begin(); it != m_arpCache.end();)
    {
        if (!force && (it->second->IsPermanent() || it->second->IsAutoGenerated()))
        {
            ++it;
            continue;
        }
        delete (*it).second;
        it = m_arpCache.erase(it);
    }
    if (m_waitReplyTimer.IsPending())
    {
