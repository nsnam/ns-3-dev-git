...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddAttribute ("EcnMode", "Determines the mode of ECN",
                   ...
                   MakeEnumChecker (EcnMode_t::NoEcn, "NoEcn",
                                    ...
                                    EcnMode_t::EcnpTry, "EcnpTry"))    // Added New ECN mode for ECN+/TryOnce
    .AddTraceSource ("RTO",
      ...
      if(m_ecnMode == EcnMode_t::EcnpTry)
        {
          SendEmptyPacket ((TcpHeader::SYN | TcpHeader::ECE | TcpHeader::CWR),false);
           m_tcb->m_ecnState = TcpSocketState::ECN_IDLE;    // The sender is ECN capable and have to be able to understand congestion marking in IP header in the SYN+ACK Packet received next
        }
      else if (m_ecnMode == EcnMode_t::ClassicEcn)   //New ECNMode-ECN+/TryOnce also included
        {
          SendEmptyPacket ((TcpHeader::SYN | TcpHeader::ECE | TcpHeader::CWR),false);
      ...
      else
        {
          ...
          m_tcb->m_ecnState = TcpSocketState::ECN_DISABLED;    // Because sender is not  ECN capable
        }
                          ...
                          Ptr<Ipv4Interface> incomingInterface)
{
  ...
  if (!IsValidTcpSegment (tcpHeader.GetSequenceNumber (), bytesRemoved,
   ...
   bool hassyn1=tcpHeader.GetFlags () & TcpHeader::SYN;
   bool hasack1= tcpHeader.GetFlags () & TcpHeader::ACK;
  if ((header.GetEcn() == Ipv4Header::ECN_CE && m_ecnCESeq < tcpHeader.GetSequenceNumber() ) || (header.GetEcn() == Ipv4Header::ECN_CE  && hassyn1 && hasack1 )) // Changing the ECN State on receiving CE marked SYN+ACK Packet also
    {
  ...
  else if (tcpflags & TcpHeader::SYN && !(tcpflags & TcpHeader::ACK))
    {
      ...
      if (m_ecnMode == EcnMode_t::EcnpTry && (tcpflags & (TcpHeader::CWR | TcpHeader::ECE)) == (TcpHeader::CWR | TcpHeader::ECE)) // ECN mode is EcnpTry, send SYN+ACK with ect(0)
         ...
         && m_tcb->m_nextTxSequence + SequenceNumber32 (1) == tcpHeader.GetAckNumber ()) 
       {
        ...
        if(m_tcb->m_ecnState == TcpSocketState::ECN_CE_RCVD)
         { 
          if((m_ecnMode == EcnMode_t::EcnpTry) && (tcpflags & (TcpHeader::ECE))) // Received SYN+ACK with CE. Set the ECN state of the receiver to sending_ECE and send an ACK+ECE Packet.
           {
         ...
         { 
           ...
           */
          if ((m_ecnMode == EcnMode_t::ClassicEcn || m_ecnMode == EcnMode_t::EcnpTry) && (tcpflags & (TcpHeader::ECE)))
           {
                               ...
                               const Address& fromAddress, const Address& toAddress)
{
  ...
  if (tcpflags == 0
   ...
   { 
       ...
       if (m_ecnMode == EcnMode_t::EcnpTry && (tcpHeader.GetFlags () & TcpHeader::ECE))
        {
          /* If the tcp responder get the feedback that first SYN/ACK suffers CE mark,
           then set the tcp responder IW to 1 SMSS and send a SYN/ACK again with not-ect in IP header
           If the tcp responder is the receiver, we reduce the congestion window but it doesn't affect the congestion in any way.
          */
          m_tcb->m_cWnd = 1;
          m_tcb->m_cWndInfl = m_tcb->m_cWnd;
          m_retxEvent.Cancel();
    ...
    { // Probably the peer lost my SYN+ACK
      ...
      else if (m_ecnMode == EcnMode_t::EcnpTry && (tcpHeader.GetFlags () & (TcpHeader::CWR | TcpHeader::ECE)) == (TcpHeader::CWR | TcpHeader::ECE))
        {
          NS_LOG_INFO ("Received ECN SYN packet");
          NS_LOG_DEBUG (TcpSocketState::EcnStateName[m_tcb->m_ecnState] << " -> ECN_IDLE");
          m_tcb->m_ecnState = TcpSocketState::ECN_IDLE;
...
void
TcpSocketBase::SendEmptyPacket (uint8_t flags,bool markect)
{
  ...
  if (m_retxEvent.IsExpired () && (hasSyn || hasFin) && !isAck )
    { // Retransmit SYN / SYN+ACK / FIN / FIN+ACK to guard against lost
      ...
      m_retxEvent = Simulator::Schedule (m_rto, &TcpSocketBase::SendEmptyPacket, this, flags,false);
    }
}
                             ...
                             const Address& fromAddress, const Address& toAddress)
{
   ...
   * packet and the traffic is ECN Capable
   */
  if (m_ecnMode == EcnMode_t::EcnpTry && (h.GetFlags () & (TcpHeader::CWR | TcpHeader::ECE)) == (TcpHeader::CWR | TcpHeader::ECE))
    {
      m_tcb->m_ecnState = TcpSocketState::ECN_IDLE;
      NS_LOG_DEBUG (TcpSocketState::EcnStateName[m_tcb->m_ecnState] << " -> ECN_IDLE");
      SendEmptyPacket ((TcpHeader::SYN | TcpHeader::ACK | TcpHeader::ECE),true);      
    }
  else if (m_ecnMode == EcnMode_t::ClassicEcn && (h.GetFlags () & (TcpHeader::CWR | TcpHeader::ECE)) == (TcpHeader::CWR | TcpHeader::ECE))
    {
      SendEmptyPacket ((TcpHeader::SYN | TcpHeader::ACK | TcpHeader::ECE),false);
      NS_LOG_DEBUG (TcpSocketState::EcnStateName[m_tcb->m_ecnState] << " -> ECN_IDLE");
      m_tcb->m_ecnState = TcpSocketState::ECN_IDLE;
