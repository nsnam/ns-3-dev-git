...
int64_t
RipNg::AssignStreams(int64_t stream)
{
...
void
RipNg::NotifyInterfaceUp(uint32_t i)
{
...
void
RipNg::NotifyInterfaceDown(uint32_t interface)
{
...
void
RipNg::NotifyAddAddress(uint32_t interface, Ipv6InterfaceAddress address)
{
...
void
RipNg::NotifyRemoveAddress(uint32_t interface, Ipv6InterfaceAddress address)
{
    NS_LOG_FUNCTION(this << interface << address);
...
void
RipNg::NotifyAddRoute(Ipv6Address dst,
                      Ipv6Prefix mask,
                      Ipv6Address nextHop,
                      uint32_t interface,
...
void
RipNg::NotifyRemoveRoute(Ipv6Address dst,
                         Ipv6Prefix mask,
                         Ipv6Address nextHop,
                         uint32_t interface,
                         Ipv6Address prefixToUse)
...
void
RipNg::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
    NS_LOG_FUNCTION(this << stream);
...
Ptr<Ipv6Route>
RipNg::Lookup(Ipv6Address dst, bool setSource, Ptr<NetDevice> interface)
{
...
void
RipNg::AddNetworkRouteTo(Ipv6Address network,
...
void
RipNg::AddNetworkRouteTo(Ipv6Address network, Ipv6Prefix networkPrefix, uint32_t interface)
{
...
void
RipNg::InvalidateRoute(RipNgRoutingTableEntry* route)
{
    ...
    for (auto it = m_routes.begin(); it != m_routes.end(); it++)
    {
        if (it->first == route)
        {
            ...
            it->second =
                Simulator::Schedule(m_garbageCollectionDelay, &RipNg::DeleteRoute, this, route);
            return;
        }
    }
...
void
RipNg::DeleteRoute(RipNgRoutingTableEntry* route)
{
    NS_LOG_FUNCTION(this << *route);
...
void
RipNg::Receive(Ptr<Socket> socket)
{
...
void
RipNg::HandleRequests(RipNgHeader requestHdr,
                      ...
                      uint8_t hopLimit)
{
    ...
    if (rtes.size() == 1 && senderAddress.IsLinkLocal())
    {
        if (rtes.begin()->GetPrefix() == Ipv6Address::GetAny() &&
            rtes.begin()->GetPrefixLen() == 0 && rtes.begin()->GetRouteMetric() == m_linkDown)
        {
            ...
            if (m_interfaceExclusions.find(incomingInterface) == m_interfaceExclusions.end())
            {
                ...
                uint16_t maxRte =
                    (mtu - Ipv6Header().GetSerializedSize() - UdpHeader().GetSerializedSize() -
                     RipNgHeader().GetSerializedSize()) /
                    RipNgRte().GetSerializedSize();
                ...
                for (auto rtIter = m_routes.begin(); rtIter != m_routes.end(); rtIter++)
                {
                    ...
                    bool isDefaultRoute =
                        ((rtIter->first->GetDestNetwork() == Ipv6Address::GetAny()) &&
...
void
RipNg::HandleResponses(RipNgHeader hdr,
                       Ipv6Address senderAddress,
                       uint32_t incomingInterface,
                       uint8_t hopLimit)
{
    ...
    for (auto iter = rtes.begin(); iter != rtes.end(); iter++)
    {
        ...
        {
            NS_LOG_LOGIC("Ignoring an update message with malformed prefix length: "
                         << int(iter->GetPrefixLen()));
            return;
        }
        if (iter->GetPrefix().IsLocalhost() || iter->GetPrefix().IsLinkLocal() ||
...
void
RipNg::DoSendRouteUpdate(bool periodic)
{
...
void
RipNg::SendTriggeredRouteUpdate()
{
    NS_LOG_FUNCTION(this);
...
void
RipNg::SendUnsolicitedRouteUpdate()
{
    NS_LOG_FUNCTION(this);
...
std::set<uint32_t>
RipNg::GetInterfaceExclusions() const
{
    return m_interfaceExclusions;
}

void
RipNg::SetInterfaceExclusions(std::set<uint32_t> exceptions)
{
    NS_LOG_FUNCTION(this);
...
uint8_t
RipNg::GetInterfaceMetric(uint32_t interface) const
{
    NS_LOG_FUNCTION(this << interface);
...
void
RipNg::SetInterfaceMetric(uint32_t interface, uint8_t metric)
{
...
void
RipNg::AddDefaultRouteTo(Ipv6Address nextHop, uint32_t interface)
{
    NS_LOG_FUNCTION(this << interface);
...
RipNgRoutingTableEntry::RipNgRoutingTableEntry()
    : m_tag(0),
      m_metric(0),
      m_status(RIPNG_INVALID),
      m_changed(false)
{
}

RipNgRoutingTableEntry::RipNgRoutingTableEntry(Ipv6Address network,
                                               Ipv6Prefix networkPrefix,
                                               Ipv6Address nextHop,
                                               uint32_t interface,
                                               Ipv6Address prefixToUse)
    : Ipv6RoutingTableEntry(RipNgRoutingTableEntry::CreateNetworkRouteTo(network,
                                                                         networkPrefix,
                                                                         nextHop,
                                                                         interface,
                                                                         prefixToUse)),
...
uint16_t
RipNgRoutingTableEntry::GetRouteTag() const
{
    return m_tag;
}

void
RipNgRoutingTableEntry::SetRouteMetric(uint8_t routeMetric)
{
...
uint8_t
RipNgRoutingTableEntry::GetRouteMetric() const
{
    return m_metric;
}

void
RipNgRoutingTableEntry::SetRouteStatus(Status_e status)
{
    if (m_status != status)
    {
        m_status = status;
...
RipNgRoutingTableEntry::Status_e
RipNgRoutingTableEntry::GetRouteStatus() const
{
    return m_status;
}

void
RipNgRoutingTableEntry::SetRouteChanged(bool changed)
{
    m_changed = changed;
}

bool
RipNgRoutingTableEntry::IsRouteChanged() const
{
    return m_changed;
}

std::ostream&
operator<<(std::ostream& os, const RipNgRoutingTableEntry& rte)
{
