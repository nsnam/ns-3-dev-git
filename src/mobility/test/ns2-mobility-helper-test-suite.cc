 ...
 *  4. Run simulation listening for all CourseChange events and compare actual mobility with the
 * reference
 */
  ...
  private:
    ...
    bool CheckInitialPositions()
    {
               ...
               m_reference[m_nextRefPoint].time == Seconds(0))
        {
            ...
            NS_TEST_ASSERT_MSG_NE_RETURNS_BOOL(node,
                                               nullptr,
                                               "Can't find node with id " << rp.node);
            Ptr<MobilityModel> mob = node->GetObject<MobilityModel>();
            NS_TEST_ASSERT_MSG_NE_RETURNS_BOOL(mob,
                                               nullptr,
                                               "Can't find mobility for node " << rp.node);

            double tol = 0.001;
            NS_TEST_EXPECT_MSG_EQ(AreVectorsEqual(mob->GetPosition(), rp.pos, tol),
                                  true,
                                  "Initial position mismatch for node " << rp.node);
            NS_TEST_EXPECT_MSG_EQ(AreVectorsEqual(mob->GetVelocity(), rp.vel, tol),
                                  true,
                                  "Initial velocity mismatch for node " << rp.node);
    ...
    void CourseChange(std::string context, Ptr<const MobilityModel> mobility)
    {
        ...
        const ReferencePoint& ref = m_reference[m_nextRefPoint++];
        NS_TEST_EXPECT_MSG_EQ(time, ref.time, "Time mismatch");
        NS_TEST_EXPECT_MSG_EQ(id,
                              ref.node,
                              "Node ID mismatch at time " << time.GetSeconds() << " s");

        double tol = 0.001;
        NS_TEST_EXPECT_MSG_EQ(AreVectorsEqual(pos, ref.pos, tol),
                              true,
                              "Position mismatch at time " << time.GetSeconds() << " s for node "
                                                           << id);
        NS_TEST_EXPECT_MSG_EQ(AreVectorsEqual(vel, ref.vel, tol),
                              true,
                              "Velocity mismatch at time " << time.GetSeconds() << " s for node "
                                                           << id);
    }
  ...
  public:
        ...
        : TestSuite("mobility-ns2-trace-helper", Type::UNIT)
    {
        ...
        t->SetTrace("$node_(0) set X_ 350.00000000000000\n"
                    ...
                    "$ns_ at 50.00000000000000  \"$node_(0) setdest 400.00000000000000 "
                    "50.00000000000000 1.00000000000000\"\n"
                    "$ns_ at 150.00000000000000 \"$node_(0) setdest 400.00000000000000 "
                    "150.00000000000000 4.00000000000000\"\n"
                    "$ns_ at 300.00000000000000 \"$node_(0) setdest 250.00000000000000 "
                    "150.00000000000000 3.00000000000000\"\n"
                    "$ns_ at 350.00000000000000 \"$node_(0) setdest 250.00000000000000 "
                    "50.00000000000000 1.00000000000000\"\n"
                    "$ns_ at 600.00000000000000 \"$node_(0) setdest 250.00000000000000 "
                    "1050.00000000000000 2.00000000000000\"\n"
                    "$ns_ at 900.00000000000000 \"$node_(0) setdest 300.00000000000000 "
                    "650.00000000000000 2.50000000000000\"\n");
        t->AddReferencePoint("0",
                             0.000,
                             Vector(350.000, 50.000, 0.000),
        ...
        t->AddReferencePoint("0",
                             100.000,
                             Vector(400.000, 50.000, 0.000),
                             Vector(0.000, 0.000, 0.000));
        t->AddReferencePoint("0",
                             150.000,
                             Vector(400.000, 50.000, 0.000),
                             Vector(0.000, 4.000, 0.000));
        t->AddReferencePoint("0",
                             175.000,
                             Vector(400.000, 150.000, 0.000),
                             Vector(0.000, 0.000, 0.000));
        t->AddReferencePoint("0",
                             300.000,
                             Vector(400.000, 150.000, 0.000),
        ...
        t->AddReferencePoint("0",
                             450.000,
                             Vector(250.000, 50.000, 0.000),
                             Vector(0.000, 0.000, 0.000));
        t->AddReferencePoint("0",
                             600.000,
                             Vector(250.000, 50.000, 0.000),
                             Vector(0.000, 2.000, 0.000));
        t->AddReferencePoint("0",
                             900.000,
                             Vector(250.000, 650.000, 0.000),
