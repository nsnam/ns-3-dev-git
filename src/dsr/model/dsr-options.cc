...
void
DsrOptions::SetNode(Ptr<Node> node)
{
...
bool
DsrOptions::ContainAddressAfter(Ipv4Address ipv4Address,
...
Ptr<Ipv4Route>
DsrOptions::SetRoute(Ipv4Address nextHop, Ipv4Address srcAddress)
{
...
Ipv4Address
DsrOptions::SearchNextHop(Ipv4Address ipv4Address, std::vector<Ipv4Address>& vec)
{
    NS_LOG_FUNCTION(this << ipv4Address);
    Ipv4Address nextHop;
...
Ipv4Address
DsrOptions::ReverseSearchNextHop(Ipv4Address ipv4Address, std::vector<Ipv4Address>& vec)
{
    NS_LOG_FUNCTION(this << ipv4Address);
    Ipv4Address nextHop;
    if (vec.size() == 2)
...
Ipv4Address
DsrOptions::ReverseSearchNextTwoHop(Ipv4Address ipv4Address, std::vector<Ipv4Address>& vec)
{
    NS_LOG_FUNCTION(this << ipv4Address);
...
void
DsrOptions::PrintVector(std::vector<Ipv4Address>& vec)
{
    NS_LOG_FUNCTION(this);
    /*
...
void
DsrOptions::RemoveDuplicates(std::vector<Ipv4Address>& vec)
{
    NS_LOG_FUNCTION(this);
...
Ptr<Node>
DsrOptions::GetNodeWithAddress(Ipv4Address ipv4Address)
{
    NS_LOG_FUNCTION(this << ipv4Address);
...
uint8_t
DsrOptionPad1::GetOptionNumber() const
{
    NS_LOG_FUNCTION_NOARGS();
...
uint8_t
DsrOptionPad1::Process(Ptr<Packet> packet,
                       Ptr<Packet> dsrP,
...
uint8_t
DsrOptionPadn::GetOptionNumber() const
{
    NS_LOG_FUNCTION_NOARGS();
    return OPT_NUMBER;
}
...
uint8_t
DsrOptionPadn::Process(Ptr<Packet> packet,
                       Ptr<Packet> dsrP,
                       Ipv4Address ipv4Address,
                       Ipv4Address source,
                       const Ipv4Header& ipv4Header,
...
uint8_t
DsrOptionRreq::GetOptionNumber() const
{
    NS_LOG_FUNCTION_NOARGS();
...
uint8_t
DsrOptionRreq::Process(Ptr<Packet> packet,
                       Ptr<Packet> dsrP,
                       Ipv4Address ipv4Address,
                       Ipv4Address source,
                       const Ipv4Header& ipv4Header,
                       ...
                       Ipv4Address promiscSource)
{
    ...
    if (ttl > 1)
    {
        // if the ttl value is not greater than 1, then this request will be forwarded, and we need
        // to save it in the source entry
        dupRequest = dsr->FindSourceEntry(source, targetAddress, requestId);
    }
    /*
    ...
    DsrRouteCacheEntry::IP_VECTOR ip =
    ...
    if (targetAddress == ipv4Address)
    {
        ...
        if (nodeList.empty())
        {
            ...
            m_finalRoute.clear();
            // The source is not in the hop list; retrieve it from the IP header.
        ...
        {
            // Prepend the original source (from IP header) then the intermediate hops,
            // and finally this node (the destination).
            m_finalRoute.clear();
            m_finalRoute.push_back(source); // original source from IP header
            m_finalRoute.insert(m_finalRoute.end(), nodeList.begin(), nodeList.end());
            m_finalRoute.push_back(ipv4Address); // destination (us)
        ...
        if (ReverseRoutes(m_finalRoute))
        {
            ...
            if (addRoute)
            {
                ...
                if (nextHop == "0.0.0.0")
                {
                    dsr->PacketNewRoute(dsrP, source, dst, protocol);
                    return 0;
                }
                SetRoute(nextHop, ipv4Address);
                /*
    ...
    else if (isRouteInCache && !areThereDuplicates)
    {
        ...
        /**
         * Prepend original source from IP header, then intermediate hops,
         * then the cached route to the destination.
         */
        m_finalRoute.push_back(source); // original source from IP header
        m_finalRoute.insert(m_finalRoute.end(), saveRoute.begin(), saveRoute.end());
        m_finalRoute.insert(m_finalRoute.end(), ip.begin(), ip.end());
        /*
         * Create the route entry to the rreq originator and save it to route cache, also need
         * to reverse the route
         */
        ...
        {
            // Reconstruct the route back to source: [srcAddress, intermediates..., thisNode]
            saveRoute.insert(saveRoute.begin(), source);
    ...
    if (errP->GetSize())
    {
        ...
        if ((errorSrc == source) && (unreachNode == ipv4Address))
        {
    ...
    if (ttl > 1)
    {
        ...
        dsr->ScheduleInterRequest(interP, source);
        isPromisc = false;
    }
    return rreq.GetSerializedSize();
}
...
uint8_t
DsrOptionRrep::Process(Ptr<Packet> packet,
                       Ptr<Packet> dsrP,
                       Ipv4Address ipv4Address,
                       Ipv4Address source,
                       const Ipv4Header& ipv4Header,
                       ...
                       Ipv4Address promiscSource)
{
    ...
    if (targetAddress == ipv4Address)
    {
        ...
        if (addRoute)
        {
            ...
            if (nextHop == "0.0.0.0")
            {
                dsr->PacketNewRoute(dsrP, srcAddress, dst, protocol);
                return 0;
            }
            PrintVector(nodeList);
            SetRoute(nextHop, ipv4Address);
    ...
    {
        NS_LOG_DEBUG("The length of rrep option " << (uint32_t)(rrep.GetLength() - 2));

        if ((rrep.GetLength() - 2) % 2 !=
            0) // The get length - 2 is to get aligned for the malformed header check
        {
            NS_LOG_LOGIC("Malformed header. Drop!");
            m_dropTrace(packet);
            return 0;
...
uint8_t
DsrOptionSR::GetOptionNumber() const
{
    NS_LOG_FUNCTION_NOARGS();
    return OPT_NUMBER;
}
...
uint8_t
DsrOptionSR::Process(Ptr<Packet> packet,
                     Ptr<Packet> dsrP,
                     Ipv4Address ipv4Address,
                     Ipv4Address source,
                     const Ipv4Header& ipv4Header,
                     ...
                     Ipv4Address promiscSource)
{
    NS_LOG_FUNCTION(this << packet << dsrP << ipv4Address << source << ipv4Address << ipv4Header
                         << (uint32_t)protocol << isPromisc);
    Ptr<Packet> p = packet->Copy();
    // Get the number of routers' address field
    uint8_t buf[2];
    ...
    // The route saved in the source route
    ...
    if (isPromisc)
    {
        ...
        if (destAddress != destination)
        {
            ...
            for (int32_t i = 0; i < nNodes; ++i)
            {
                ...
                // the packet
                findPassive = dsrNode->PassiveEntryCheck(packet,
                                                         source,
                                                         destination,
                                                         segsLeft,
                                                         fragmentOffset,
    ...
    else
    {
        ...
        if (optionType == 160)
        {
            ...
            NS_LOG_DEBUG("Send back ACK to the earlier hop " << ackAddress << " from us "
                                                             << ipv4Address);
            dsr->SendAck(ackId, ackAddress, source, destination, m_ipv4Route);
        }
        /*
...
uint8_t
DsrOptionRerr::DoSendError(Ptr<Packet> p,
                           DsrOptionRerrUnreachHeader& rerr,
                           uint32_t rerrSize,
                           Ipv4Address ipv4Address,
                           uint8_t protocol)
{
    ...
    // Set the route entry
    SetRoute(nextAddress, ipv4Address);
    dsr->ForwardErrPacket(rerr, newSourceRoute, nextAddress, m_ipv4Route);
    return serializedSize;
}
...
uint8_t
DsrOptionAck::GetOptionNumber() const
{
    NS_LOG_FUNCTION_NOARGS();
    return OPT_NUMBER;
}
...
uint8_t
DsrOptionAck::Process(Ptr<Packet> packet,
                      Ptr<Packet> dsrP,
                      Ipv4Address ipv4Address,
                      Ipv4Address source,
                      const Ipv4Header& ipv4Header,
