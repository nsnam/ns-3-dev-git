...
def apply_ns3header(self):
    ...
    headers_to_process = set(self.to_list(self.source))
    if hasattr(self, 'deprecated'):
        headers_to_process.update(set(self.to_list(self.deprecated)))
    for filename in headers_to_process:
    ...
    if hasattr(self, 'deprecated'):
        self.deprecated = '' # tell WAF not to process these files further
