...
NS_LOG_COMPONENT_DEFINE("MobileGamingTest");
 ...
 */
class MobileGamingTestCase : public TestCase
{
  public:
     ...
     * provided
     */
    MobileGamingTestCase(const std::string& name, const GamingParams& params);
  ...
  private:
     ...
     */
    void PacketTx(Ptr<const Packet> packet, MobileGaming::TrafficModelStage stage);
    ...
    struct TxInfo
    {
        ...
        MobileGaming::TrafficModelStage stage{
            MobileGaming::TrafficModelStage::INITIAL}; //!< traffic model stage when the packet is
                                                       //!< transmitted
    };
...
MobileGamingTestCase::MobileGamingTestCase(const std::string& name, const GamingParams& params)
    : TestCase{name},
      m_params{params}
{
}
...
void
MobileGamingTestCase::PacketTx(Ptr<const Packet> packet, MobileGaming::TrafficModelStage stage)
{
...
void
MobileGamingTestCase::ReceiveRx(std::string context, Ptr<const Packet> p, const Address& addr)
{
...
void
MobileGamingTestCase::DoSetup()
{
    NS_LOG_FUNCTION(this);
    // LogComponentEnable("MobileGamingTest", LOG_LEVEL_ALL);
    // LogComponentEnable("MobileGaming", LOG_LEVEL_ALL);
    ...
    ApplicationHelper sourceHelper(MobileGaming::GetTypeId());
...
MobileGamingTestCase::DoRun()
{
    ...
    const auto allGamingPackets =
        std::all_of(m_sent.cbegin() + 1, m_sent.cend() - 1, [](const auto& tx) {
            return tx.stage == MobileGaming::TrafficModelStage::GAMING;
        });
...
{
  public:
    MobileGamingTestSuite();
};

MobileGamingTestSuite::MobileGamingTestSuite()
    : TestSuite("applications-mobile-gaming", Type::UNIT)
{
    AddTestCase(
        new MobileGamingTestCase("Check real-time mobile gaming traffic for DL status-sync",
                                 {0, 20, 500, 600, 50, 11.0, MilliSeconds(13), MicroSeconds(3700)}),
        TestCase::Duration::QUICK);
    AddTestCase(
        new MobileGamingTestCase("Check real-time mobile gaming traffic for UL status-sync",
                                 {0, 20, 400, 550, 38, 3.7, MilliSeconds(15), MicroSeconds(5700)}),
        TestCase::Duration::QUICK);
    AddTestCase(new MobileGamingTestCase(
                    "Check real-time mobile gaming traffic for DL lockstep",
                    {0, 80, 140, 150, 210, 35.0, MilliSeconds(28), MicroSeconds(4200)}),
                TestCase::Duration::QUICK);
    AddTestCase(
        new MobileGamingTestCase("Check real-time mobile gaming traffic for UL lockstep",
                                 {0, 80, 50, 60, 92, 38.0, MilliSeconds(22), MicroSeconds(3400)}),
        TestCase::Duration::QUICK);
}

static MobileGamingTestSuite g_MobileGamingTestSuite; //!< Static variable for test initialization
