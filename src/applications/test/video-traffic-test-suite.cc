...
NS_LOG_COMPONENT_DEFINE("VideoTrafficTest");
 ...
 */
std::string
ModelToString(VideoTraffic::TrafficModelClassIdentifier model)
{
    switch (model)
    {
    case VideoTraffic::CUSTOM:
        return "Custom";
    case VideoTraffic::BUFFERED_VIDEO_1:
        return "BV1";
    case VideoTraffic::BUFFERED_VIDEO_2:
        return "BV2";
    case VideoTraffic::BUFFERED_VIDEO_3:
        return "BV3";
    case VideoTraffic::BUFFERED_VIDEO_4:
        return "BV4";
    case VideoTraffic::BUFFERED_VIDEO_5:
        return "BV5";
    case VideoTraffic::BUFFERED_VIDEO_6:
        return "BV6";
    case VideoTraffic::MULTICAST_VIDEO_1:
        return "MC1";
    case VideoTraffic::MULTICAST_VIDEO_2:
        return "MC2";
    default:
        return "";
    }
 ...
 *
 * @param weibullScale the scale of the Weibull distribution used to generate random frame sizes
 * @param weibullShape the shape of the Weibull distribution used to generate random frame sizes
 * @return the expected average frame size
 */
uint32_t
GetAverageframeSize(double weibullScale, double weibullShape)
{
    auto weibull = CreateObject<WeibullRandomVariable>();
    return static_cast<uint32_t>(WeibullRandomVariable::GetMean(weibullScale, weibullShape));
}
 ...
 */
bool
IsMulticast(VideoTraffic::TrafficModelClassIdentifier model)
{
    return ((model == VideoTraffic::MULTICAST_VIDEO_1) ||
            (model == VideoTraffic::MULTICAST_VIDEO_2));
}
 ...
 * @ingroup tests
 *
 * Video traffic test, check for the expected inter frame interval, bit rate and packets inter
 * arrivals.
 */
class VideoTrafficTestCase : public TestCase
{
  public:
    /**
     * Constructor
     ...
     */
    VideoTrafficTestCase(VideoTraffic::TrafficModelClassIdentifier model,
                         const std::string& protocol,
                         double expectedBitRate,
                         std::optional<VideoTraffic::TrafficModelParameters> parameters = {},
  ...
  private:
    ...
    VideoTraffic::TrafficModelClassIdentifier m_model; //!< Selected buffered video traffic model
    std::string m_protocol;                            //!< Udp or Tcp protocol
    std::optional<VideoTraffic::TrafficModelParameters>
...
VideoTrafficTestCase::VideoTrafficTestCase(
    VideoTraffic::TrafficModelClassIdentifier model,
    const std::string& protocol,
    double expectedBitRate,
    std::optional<VideoTraffic::TrafficModelParameters> parameters,
    std::optional<uint32_t> txBufferSizeLimit)
...
void
VideoTrafficTestCase::PacketSent(Ptr<const Packet> packet, Time latency)
{
...
void
VideoTrafficTestCase::FrameGenerated(uint32_t size)
{
...
void
VideoTrafficTestCase::ReceiveRx(std::string context, Ptr<const Packet> p, const Address& addr)
{
...
void
VideoTrafficTestCase::DoSetup()
{
    NS_LOG_FUNCTION(this);

    // LogComponentEnable("VideoTrafficTest", LOG_LEVEL_ALL);
    // LogComponentEnable("VideoTraffic", LOG_LEVEL_ALL);
    ...
    ApplicationHelper sourceHelper(VideoTraffic::GetTypeId());
    ...
    if (m_parameters)
    {
        ...
        sourceHelper.SetAttribute("CustomWeibullScale", DoubleValue(m_parameters->weibullScale));
        sourceHelper.SetAttribute("CustomWeibullShape", DoubleValue(m_parameters->weibullShape));
    }
    ...
    Config::ConnectWithoutContext(
        "/NodeList/*/$ns3::Node/ApplicationList/*/$ns3::VideoTraffic/TxWithLatency",
        MakeCallback(&VideoTrafficTestCase::PacketSent, this));

    Config::ConnectWithoutContext(
        "/NodeList/*/$ns3::Node/ApplicationList/*/$ns3::VideoTraffic/VideoFrameGenerated",
        MakeCallback(&VideoTrafficTestCase::FrameGenerated, this));

    Config::Connect("/NodeList/*/ApplicationList/*/$ns3::PacketSink/Rx",
                    MakeCallback(&VideoTrafficTestCase::ReceiveRx, this));
}

void
VideoTrafficTestCase::DoRun()
{
    NS_LOG_FUNCTION(this);
    ...
    const auto averageFrameSize =
        (m_parameters)
            ? GetAverageframeSize(m_parameters->weibullScale, m_parameters->weibullShape)
            : GetAverageframeSize(VideoTraffic::m_trafficModels.at(m_model).weibullScale,
                                  VideoTraffic::m_trafficModels.at(m_model).weibullShape);
...
{
  public:
    VideoTrafficTestSuite();
};

VideoTrafficTestSuite::VideoTrafficTestSuite()
    : TestSuite("applications-video-traffic", Type::UNIT)
{
    for (const auto& protocol : {"Tcp", "Udp"})
    {
        AddTestCase(new VideoTrafficTestCase(VideoTraffic::BUFFERED_VIDEO_1, protocol, 2.0),
                    TestCase::Duration::QUICK);
        AddTestCase(new VideoTrafficTestCase(VideoTraffic::BUFFERED_VIDEO_2, protocol, 4.0),
                    TestCase::Duration::QUICK);
        AddTestCase(new VideoTrafficTestCase(VideoTraffic::BUFFERED_VIDEO_3, protocol, 6.0),
                    TestCase::Duration::QUICK);
        AddTestCase(new VideoTrafficTestCase(VideoTraffic::BUFFERED_VIDEO_4, protocol, 8.0),
                    TestCase::Duration::QUICK);
        AddTestCase(new VideoTrafficTestCase(VideoTraffic::BUFFERED_VIDEO_5, protocol, 10.0),
                    TestCase::Duration::QUICK);
        AddTestCase(new VideoTrafficTestCase(VideoTraffic::BUFFERED_VIDEO_6, protocol, 15.6),
                    TestCase::Duration::QUICK);
    }
    AddTestCase(new VideoTrafficTestCase(VideoTraffic::MULTICAST_VIDEO_1, "Udp", 3.0),
                TestCase::Duration::QUICK);
    AddTestCase(new VideoTrafficTestCase(VideoTraffic::MULTICAST_VIDEO_2, "Udp", 6.0),
                TestCase::Duration::QUICK);
    AddTestCase(new VideoTrafficTestCase(VideoTraffic::CUSTOM,
                                         "Udp",
                                         5.0,
...
static VideoTrafficTestSuite g_videoTrafficTestSuite; //!< Static variable for test initialization
