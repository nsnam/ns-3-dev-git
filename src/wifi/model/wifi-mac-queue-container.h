...
template<>
struct
std::hash<ns3::WifiContainerQueueId>
{
  /**
   * The functor.
   ...
   */
  std::size_t
  operator () (ns3::WifiContainerQueueId queueId) const;
};
...
private:
  ...
  mutable std::unordered_map<WifiContainerQueueId, ContainerQueue> m_queues; //!< the container queues
  mutable ContainerQueue m_expiredQueue; //!< queue storing MPDUs with expired lifetime
  mutable std::unordered_map<WifiContainerQueueId, uint32_t> m_nBytesPerQueue; //!< size in bytes of the container queues
};
