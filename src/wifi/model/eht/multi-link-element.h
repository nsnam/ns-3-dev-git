...
struct CommonInfoBasicMle
{
  ...
  std::optional<MediumSyncDelayInfo> m_mediumSyncDelayInfo; //!< Medium Synchronization Delay Information
...
public:
  ...
  private:
    ...
    std::unique_ptr<Header> m_staProfile; /**< STA Profile field, containing the frame body of a
                                               frame of the same type as the frame containing the
                                               Multi-Link Element */
  };
...
private:
   ...
   * Link Info field
   */
  std::vector<PerStaProfileSubelement> m_perStaProfileSubelements; //!< Per-STA Profile Subelements
};
