...
class AdvancedEmlsrManager : public DefaultEmlsrManager
{
  ...
  protected:
    ...
    /**
     * Check whether it is possible to gain channel access on the given link within the given delay.
     *
     * @param linkId the ID of the given link
     * @param delay the given delay
     * @return a pair indicating whether channel access is possible and the value returned by the
     *         GetExpectedAccessWithin() function of the channel access manager
     */
    std::pair<bool, WifiExpectedAccessReason> GetAccessPossibleWithin(linkId_t linkId,
                                                                      const Time& delay);
