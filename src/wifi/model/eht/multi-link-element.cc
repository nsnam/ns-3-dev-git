...
CommonInfoBasicMle::Serialize (Buffer::Iterator& start) const
{
  ...
  if (m_emlCapabilities.has_value ())
    {
      uint16_t val = m_emlCapabilities->emlsrSupport
                     | (m_emlCapabilities->emlsrPaddingDelay << 1)
                     | (m_emlCapabilities->emlsrTransitionDelay << 4)
                     | (m_emlCapabilities->emlmrSupport << 7)
                     | (m_emlCapabilities->emlmrDelay << 8)
                     | (m_emlCapabilities->transitionTimeout << 11);
      start.WriteHtolsbU16 (val);
    }
  if (m_mldCapabilities.has_value ())
    {
      uint16_t val = m_mldCapabilities->maxNSimultaneousLinks
                     | (m_mldCapabilities->srsSupport << 4)
                     | (m_mldCapabilities->tidToLinkMappingSupport << 5)
                     | (m_mldCapabilities->freqSepForStrApMld << 7)
                     | (m_mldCapabilities->aarSupport << 12);
      start.WriteHtolsbU16 (val);
    }
}
...
CommonInfoBasicMle::Deserialize (Buffer::Iterator start, uint16_t presence)
{
  ...
  NS_ABORT_MSG_IF (count != length,
                   "Common Info Length (" << +length << ") differs "
                   "from actual number of bytes read (" << +count << ")");
  return count;
}
...
MultiLinkElement::PerStaProfileSubelement::PerStaProfileSubelement (Variant variant, WifiMacType frameType)
  : m_variant (variant),
    m_frameType (frameType),
    m_staControl (0)
{
...
MultiLinkElement::PerStaProfileSubelement::PerStaProfileSubelement (const PerStaProfileSubelement& perStaProfile)
    ...
    m_staMacAddress (perStaProfile.m_staMacAddress)
{
  ...
  if (perStaProfile.HasAssocRequest ())
    {
      m_staProfile = std::make_unique<MgtAssocRequestHeader> (*static_cast<MgtAssocRequestHeader*> (perStaProfile.m_staProfile.get ()));
    }
  else if (perStaProfile.HasReassocRequest ())
    {
      m_staProfile = std::make_unique<MgtReassocRequestHeader> (*static_cast<MgtReassocRequestHeader*> (perStaProfile.m_staProfile.get ()));
    }
  else if (perStaProfile.HasAssocResponse ())
    {
      m_staProfile = std::make_unique<MgtAssocResponseHeader> (*static_cast<MgtAssocResponseHeader*> (perStaProfile.m_staProfile.get ()));
    }
}
...
void
MultiLinkElement::PerStaProfileSubelement::SetAssocRequest (const std::variant<MgtAssocRequestHeader, MgtReassocRequestHeader>& assoc)
{
  switch (m_frameType)
    {
    case WIFI_MAC_MGT_ASSOCIATION_REQUEST:
      m_staProfile = std::make_unique<MgtAssocRequestHeader> (std::get<MgtAssocRequestHeader> (assoc));
      break;
    case WIFI_MAC_MGT_REASSOCIATION_REQUEST:
      m_staProfile = std::make_unique<MgtReassocRequestHeader> (std::get<MgtReassocRequestHeader> (assoc));
      break;
    default:
...
void
MultiLinkElement::PerStaProfileSubelement::SetAssocRequest (std::variant<MgtAssocRequestHeader, MgtReassocRequestHeader>&& assoc)
{
  switch (m_frameType)
    {
    case WIFI_MAC_MGT_ASSOCIATION_REQUEST:
      m_staProfile = std::make_unique<MgtAssocRequestHeader> (std::move (std::get<MgtAssocRequestHeader> (assoc)));
      break;
    case WIFI_MAC_MGT_REASSOCIATION_REQUEST:
      m_staProfile = std::make_unique<MgtReassocRequestHeader> (std::move (std::get<MgtReassocRequestHeader> (assoc)));
      break;
    default:
      NS_ABORT_MSG ("Invalid frame type: " << m_frameType);
    }
  ...
  std::visit ([this, &start](auto&& arg)
              {
                ...
                if constexpr (std::is_same_v<T, std::monostate>)
                  ...
                  {
                    uint16_t mlControl = static_cast<uint8_t> (GetVariant ())
                                         + (arg.GetPresenceBitmap () << 4);
...
MultiLinkElement::DeserializeInformationField (Buffer::Iterator start, uint16_t length)
{
  ...
  uint8_t nBytes = std::visit ([&i, &presence](auto&& arg) -> uint8_t
                               {
                                 using T = std::decay_t<decltype(arg)>;
                                 if constexpr (std::is_same_v<T, std::monostate>)
                                   {
