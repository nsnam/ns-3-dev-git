...
const HeRu::SubcarrierGroups HeRu::m_heRuSubcarrierGroups = {
    ...
    {{MHz_t{20}, HeRu::RU_26_TONE},
     {/* 1 */ {{-121, -96}},
      ...
      /* 8 */ {{70, 95}},
      /* 9 */ {{96, 121}}}},
    {{MHz_t{20}, HeRu::RU_52_TONE},
     {/* 1 */ {{-121, -70}},
      /* 2 */ {{-68, -17}},
      /* 3 */ {{17, 68}},
      /* 4 */ {{70, 121}}}},
    {{MHz_t{20}, HeRu::RU_106_TONE},
     {/* 1 */ {{-122, -17}},
      /* 2 */ {{17, 122}}}},
    {{MHz_t{20}, HeRu::RU_242_TONE}, {/* 1 */ {{-122, -2}, {2, 122}}}},
    // RUs in a 40 MHz HE PPDU (Table 28-7)
    {{MHz_t{40}, HeRu::RU_26_TONE},
     {/* 1 */ {{-243, -218}},
      /* 2 */ {{-217, -192}},
      /* 3 */ {{-189, -164}},
      /* 4 */ {{-163, -138}},
    ...
    {{MHz_t{40}, HeRu::RU_52_TONE},
     {/* 1 */ {{-243, -192}},
      ...
      /* 7 */ {{138, 189}},
      /* 8 */ {{192, 243}}}},
    {{MHz_t{40}, HeRu::RU_106_TONE},
     {/* 1 */ {{-243, -138}},
      /* 2 */ {{-109, -4}},
      /* 3 */ {{4, 109}},
      /* 4 */ {{138, 243}}}},
    {{MHz_t{40}, HeRu::RU_242_TONE},
     {/* 1 */ {{-244, -3}},
      /* 2 */ {{3, 244}}}},
    {{MHz_t{40}, HeRu::RU_484_TONE}, {/* 1 */ {{-244, -3}, {3, 244}}}},
    // RUs in an 80 MHz HE PPDU (Table 28-8)
    {{MHz_t{80}, HeRu::RU_26_TONE},
     {/* 1 */ {{-499, -474}},
      /* 2 */ {{-473, -448}},
      /* 3 */ {{-445, -420}},
      /* 4 */ {{-419, -394}},
    ...
    {{MHz_t{80}, HeRu::RU_106_TONE},
     {/* 1 */ {{-499, -394}},
      ...
      /* 7 */ {{260, 365}},
      /* 8 */ {{394, 499}}}},
    {{MHz_t{80}, HeRu::RU_242_TONE},
     {/* 1 */ {{-500, -259}},
      /* 2 */ {{-258, -17}},
      /* 3 */ {{17, 258}},
      /* 4 */ {{259, 500}}}},
    {{MHz_t{80}, HeRu::RU_484_TONE},
     {/* 1 */ {{-500, -17}},
      /* 2 */ {{17, 500}}}},
    {{MHz_t{80}, HeRu::RU_996_TONE}, {/* 1 */ {{-500, -3}, {3, 500}}}},
};
...
std::size_t
HeRu::RuSpec::GetPhyIndex(MHz_t bw, uint8_t p20Index) const
{
    bool primary80IsLower80 = (p20Index < bw / MHz_t{40});

    if (bw < MHz_t{160} || m_ruType == HeRu::RU_2x996_TONE ||
...
std::size_t
HeRu::GetNRus(MHz_t bw, RuType ruType)
{
    if (bw == MHz_t{160} && ruType == RU_2x996_TONE)
    {
        return 1;
    }
    ...
    auto it = m_heRuSubcarrierGroups.find({(bw == MHz_t{160} ? MHz_t{80} : bw), ruType});
    ...
    return (bw == MHz_t{160} ? 2 : 1) * it->second.size();
}

std::vector<HeRu::RuSpec>
HeRu::GetRusOfType(MHz_t bw, HeRu::RuType ruType)
{
...
std::vector<HeRu::RuSpec>
HeRu::GetCentral26TonesRus(MHz_t bw, HeRu::RuType ruType)
{
...
HeRu::GetSubcarrierGroup(MHz_t bw, RuType ruType, std::size_t phyIndex)
{
    if (ruType == HeRu::RU_2x996_TONE) // handle special case of RU covering 160 MHz channel
    {
        NS_ABORT_MSG_IF(bw != MHz_t{160}, "2x996 tone RU can only be used on 160 MHz band");
    ...
    int16_t shift = (bw == MHz_t{160}) ? -512 : 0;
    if (bw == MHz_t{160} && phyIndex > (numRus / 2))
    {
...
bool
HeRu::DoesOverlap(MHz_t bw, RuSpec ru, const std::vector<RuSpec>& v)
{
    // A 2x996-tone RU spans 160 MHz, hence it overlaps with any other RU
    if (bw == MHz_t{160} && ru.GetRuType() == RU_2x996_TONE && !v.empty())
    {
        return true;
    }
...
bool
HeRu::DoesOverlap(MHz_t bw, RuSpec ru, const SubcarrierGroup& toneRanges, uint8_t p20Index)
{
    for (const auto& range : toneRanges)
    {
        if (bw == MHz_t{160} && ru.GetRuType() == RU_2x996_TONE)
        {
            return true;
        }
...
HeRu::RuSpec
HeRu::FindOverlappingRu(MHz_t bw, RuSpec referenceRu, RuType searchedRuType)
{
...
HeRu::RuType
HeRu::GetEqualSizedRusForStations(MHz_t bandwidth,
                                  ...
                                  std::size_t& nCentral26TonesRus)
{
    ...
    for (auto& ru : m_heRuSubcarrierGroups)
    {
        ...
        else if (bandwidth == MHz_t{160} && ru.first.first == MHz_t{80} &&
    ...
    if (nRusAssigned == 0)
    {
        NS_ABORT_IF(bandwidth != MHz_t{160} || nStations != 1);
        nRusAssigned = 1;
    ...
    case RU_52_TONE:
    case RU_106_TONE:
        if (bandwidth == MHz_t{20})
        {
            nCentral26TonesRus = 1;
        }
        else if (bandwidth == MHz_t{40})
        {
    ...
    case RU_242_TONE:
    case RU_484_TONE:
        nCentral26TonesRus = (bandwidth >= MHz_t{80} ? 1 : 0);
        break;
    default:
        nCentral26TonesRus = 0;
    }

    if (bandwidth == MHz_t{160})
    {
        nCentral26TonesRus *= 2;
    }
