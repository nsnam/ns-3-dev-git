...
{
    return ((m_channelWidth >= MHz_t{40}) && !IsMu() && (GetModulationClass() < WIFI_MOD_CLASS_HT));
}
...
void
WifiTxVector::SetChannelWidth(MHz_t channelWidth)
{
    m_channelWidth = channelWidth;
}
...
WifiTxVector::IsSigBCompression() const
{
    ...
    return (IsDlMuMimo() && !IsDlOfdma()) ||
           ((m_muUserInfos.size() == 1) && (m_channelWidth >= MHz_t{160}) &&
...
{
    if (!IsDlMu() || (m_channelWidth < MHz_t{80}))
    {
        return std::nullopt;
    }
...
WifiTxVector::DeriveRuAllocation(uint8_t p20Index) const
{
    ...
    for (const auto& [ru, staIds] : orderedMap)
    {
        ...
        const auto rusPerSubchannel =
            HeRu::GetRusOfType(ruBw > MHz_t{20} ? ruBw : MHz_t{20}, ruType);
        ...
        const auto primary80IsLower80 = (p20Index < m_channelWidth / MHz_t{40});
        const auto isLow80 =
            (primary80IsLower80 && isPrimary80MHz) || (!primary80IsLower80 && !isPrimary80MHz);
        if ((m_channelWidth > MHz_t{80}) && !isLow80 && (ruIndex > 19))
        {
            // take into account the center 26-tone RU in the high 80 MHz
            ruIndex--;
        }
        if ((m_channelWidth > MHz_t{80}) && !isLow80 && (ruType != HeRu::GetRuType(m_channelWidth)))
        {
            ...
            // the low 80 MHz
            ruIndex += HeRu::GetRusOfType(MHz_t{80}, ruType).size();
        }
