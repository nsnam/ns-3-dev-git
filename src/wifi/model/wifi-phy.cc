    ...
    static TypeId tid =
        TypeId("ns3::WifiPhy")
            ...
            .AddAttribute("Frequency",
                          ...
                          TypeId::ATTR_GET,
                          MHzValue(0_MHz),
                          MakeMHzAccessor(&WifiPhy::GetFrequency),
                          MakeMHzChecker())
            ...
            .AddAttribute(
                ...
                TypeId::ATTR_GET,
                MHzValue(0_MHz),
                MakeMHzAccessor(&WifiPhy::GetChannelWidth),
                MakeMHzChecker(5_MHz, 320_MHz))
            .AddAttribute(
                "Primary20MHzIndex",
            ...
            .AddAttribute(
                ...
                MHzValue(MHz_t{0}),
                MakeMHzAccessor(&WifiPhy::m_maxRadioBw),
                MakeMHzChecker())
            .AddAttribute("RxSensitivity",
                          "The energy of a received signal should be higher than "
                          "this threshold for the PHY to detect the signal. "
            ...
            .AddAttribute("CcaEdThreshold",
                          ...
                          "this threshold in the primary channel to allow the PHY layer "
            ...
            .AddAttribute("TxPowerEnd",
                          "Maximum available transmission level.",
                          dBmValue(16.0206_dBm),
                          MakedBmAccessor(&WifiPhy::SetTxPowerEnd, &WifiPhy::GetTxPowerEnd),
                          MakedBmChecker())
            .AddAttribute("TxPowerStart",
                          "Minimum available transmission level.",
                          dBmValue(16.0206_dBm),
...
WifiPhy::GetPower(uint8_t powerLevel) const
{
    ...
    {
        power += dB_t{(powerLevel - WIFI_MIN_TX_PWR_LEVEL) *
                      (m_txPowerEnd.in_dBm() - m_txPowerBase.in_dBm()) / (m_nTxPowerLevels - 1)};
    }
    return power;
}
...
WifiPhy::Configure80211p()
{
    ...
    else if (GetChannelWidth() == MHz_t{5})
    {
...
{
    return m_operatingChannel.GetPrimaryChannelIndex(MHz_t{20});
}
...
MHz_t
WifiPhy::GetTxBandwidth(WifiMode mode, MHz_t maxAllowedWidth) const
{
...
WifiPhy::SetOperatingChannel(const WifiPhyOperatingChannel& channel)
{
    ...
    for (std::size_t segmentId = 0; segmentId < channel.GetNSegments(); ++segmentId)
    {
        segments.emplace_back(channel.GetNumber(segmentId),
                              channel.GetWidth(segmentId).in_MHz(),
                              channel.GetPhyBand(),
                              channel.GetPrimaryChannelIndex(MHz_t{20}));
    }
...
{
    ChannelSegmentsWithUnits segments;
    for (const auto& [number, width, band, primary20] : channelSegments)
    {
        segments.emplace_back(number, MHz_t{width}, band, primary20);
    }
    SetOperatingChannel(segments);
}

void
WifiPhy::SetOperatingChannel(const ChannelSegmentsWithUnits& channelSegments)
{
    ...
    if (IsInitialized())
    {
        ...
        if (delay.value().IsStrictlyPositive())
        {
            ...
            void (WifiPhy::*fp)(const ChannelSegmentsWithUnits&) = &WifiPhy::SetOperatingChannel;
...
WifiPhy::DoChannelSwitch()
{
    ...
    if (const auto chWidth = GetChannelWidth();
        (m_maxRadioBw != MHz_t{0}) && (chWidth > m_maxRadioBw))
    {
        // throw an exception instead of using NS_ABORT_MSG for unit testing this code
        throw std::runtime_error("Attempting to set a " + chWidth.str() +
                                 " channel on a station only supporting " + m_maxRadioBw.str() +
                                 " operation");
    }
...
WifiPhy::NotifyTxBegin(const WifiConstPsduMap& psdus, Watt_t txPower)
{
    if (!m_phyTxBeginTrace.IsEmpty())
    {
        for (const auto& psdu : psdus)
        {
            ...
            {
                m_phyTxBeginTrace(mpdu->GetProtocolDataUnit(), txPower.in_Watt());
            }
        }
    }
}
            ...
            for (size_t i = 0; i < nMpdus;)
            {
                if (statusPerMpdu.at(i)) // packet received without error, hand over to sniffer
                {
                    m_phyMonitorSniffRxTrace(psdu->GetAmpduSubframe(i),
                                             static_cast<uint16_t>(channelFreq.in_MHz()),
                                             txVector,
                                             aMpdu,
                                             {
                                                 .signal = signalNoise.signal.in_dBm(),
                                                 .noise = signalNoise.noise.in_dBm(),
                                             },
                                             staId);
                }
                ++i;
                              ...
                              uint16_t staId)
{
    ...
    else
    {
        ...
        {
            aMpdu.type = NORMAL_MPDU;
            m_phyMonitorSniffTxTrace(psdu->GetPacket(),
                                     channelFreq.in_MHz(),
                                     txVector,
                                     aMpdu,
                                     staId);
...
WifiPhy::Send(const WifiConstPsduMap& psdus, const WifiTxVector& txVector)
{
    ...
    {
        m_phyTxPsduBeginTrace(psdus, txVector, txPower.in_Watt());
    }
    for (const auto& psdu : psdus)
    {
...
WifiPhy::GetTxPowerForTransmission(Ptr<const WifiPpdu> ppdu) const
{
    ...
    const auto txPowerDbmPerMhz =
        dBm_per_MHz_t::AveragePsd(txPower + GetTxGain(),
                                  channelWidth); // account for antenna gain since EIRP
    NS_LOG_INFO("txPower=" << txPower << " with txPowerDbmPerMhz=" << txPowerDbmPerMhz << " over "
                           << channelWidth);
    txPower = std::min(txPowerDbmPerMhz, m_powerDensityLimit).OverBandwidth(channelWidth);
...
{
    os << "SNR:" << RatioToDb(rxSignalInfo.snr) << ", RSSI:" << rxSignalInfo.rssi;
    return os;
}

uint8_t
WifiPhy::GetPrimaryChannelNumber(MHz_t primaryChannelWidth) const
{
