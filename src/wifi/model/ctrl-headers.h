...
/// struct to identify an unavailability period
struct WifiUnavailPeriod
{
    Time start;              ///< unavailability period start
    std::optional<Time> end; ///< unavailability period end, unless the duration is indefinite
};
...
/**
 * @ingroup wifi
 * @brief Feedback User Info field of Trigger frames.
 *
 * The Feedback User Info field is a User Info field that does not carry user specific information
 * but carries feedback information. The Feedback User Info field is identified by an AID12 value of
 * 2008 and is optionally present in a BSRP NTB Trigger frame that is generated by a UHR non-AP STA.
 *
 * Only the Unsolicited Unavailability feedback type is supported for now.
 */
class CtrlTriggerFeedbackUserInfoField
{
  public:
    /**
     * Feedback Type subfield encoding (Table 9-46m5 802.11bn D1.0)
     */
    enum class Type : uint8_t
    {
        UNSOLICITED_UNAVAILABILITY = 0,
        CO_BF = 2,
        CO_TDMA = 3,
        CO_SR = 4
    };
     ...
     * @param triggerVariant the Trigger frame variant
     */
    CtrlTriggerFeedbackUserInfoField(Type type = Type::UNSOLICITED_UNAVAILABILITY,
                                     TriggerFrameVariant triggerVariant = TriggerFrameVariant::UHR);

    /**
     * Copy assignment operator.
     *
     * @param other the Feedback User Info field to copy
     * @return a reference to the copied object
     */
    CtrlTriggerFeedbackUserInfoField& operator=(const CtrlTriggerFeedbackUserInfoField& other);

    /**
     * @return the expected size of this Feedback User Info field.
     */
    uint32_t GetSerializedSize() const;

    /**
     * Serialize the Feedback User Info field to the given buffer.
     *
    ...
    /**
     * Deserialize the Feedback User Info field from the given buffer.
     *
     * @param start an iterator which points to where the header should
     *        read from
     * @return Buffer::Iterator to the next available buffer
    ...
    /**
     * Print the content of this Feedback User Info field
     *
     * @param os output stream
     */
    void Print(std::ostream& os) const;
    ...
    /**
     * In case the feedback type is Unsolicited Unavailability, set the subfields of the Feedback
     * Information field to indicate that the STA is unavailable for the given period of time.
     *
     ...
     */
    void SetStaUnavailable(const WifiUnavailPeriod& unavailPeriod, bool& indefinite);

    /// In case the feedback type is Unsolicited Unavailability, set the subfields of the Feedback
    /// Information field to indicate that the STA is available.
    void SetStaAvailable();

    /// @return the unavailability period encoded in the Feedback Information field, or a null
    ///         value if such a field indicates that the STA is available. It is required that
    ///         the feedback type is Unsolicited Unavailability.
    std::optional<WifiUnavailPeriod> GetUnavailability() const;

    Type m_feedbackType{}; //!< Feedback type

  private:
    TriggerFrameVariant m_triggerVariant{};         //!< Trigger variant
    std::array<uint8_t, 3> m_feedbackInformation{}; //!< Feedback Information subfield
};
...
class CtrlTriggerHeader : public Header
{
  public:
    ...
    bool IsBsrp() const;
    /**
     * Check if this is a BSRP NTB (Non-Trigger Based) Trigger frame.
     *
     * @return true if this is a BSRP NTB (Non-Trigger Based) Trigger frame,
     *         false otherwise
     */
    bool IsBsrpNtb() const;
    /**
    ...
    /**
     * Set the GI And UHR-LTF Type subfield to 3 to indicate that this is a BSRP NTB Trigger Frame.
     * It is required that the variant is (at least) UHR and the TF type is BSRP.
     */
    void SetNtb();
    /**
    ...
    /**
     * Add a Feedback User Info field of the given type to this Trigger frame and return a non-const
     * reference to it. This field can only be added to a BSRP Trigger Frame (which is then
     * automatically configured as NTB) whose variant is at least UHR.
     *
     * @param type the Feedback User Info field type
     * @return a non-const reference to the newly added Feedback User Info field
     */
    CtrlTriggerFeedbackUserInfoField& AddFeedbackUserInfoField(
        CtrlTriggerFeedbackUserInfoField::Type type =
            CtrlTriggerFeedbackUserInfoField::Type::UNSOLICITED_UNAVAILABILITY);

    /// @return the Feedback User Info field, if present
    const std::optional<CtrlTriggerFeedbackUserInfoField>& GetFeedbackUserInfo() const;
  ...
  private:
    ...
    std::optional<CtrlTriggerFeedbackUserInfoField>
        m_feedbackUserInfoField; //!< Feedback User Info field (TF variant >= UHR)
