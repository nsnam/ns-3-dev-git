...
OfdmPpdu::OfdmPpdu(Ptr<const WifiPsdu> psdu,
    ...
    : WifiPpdu(psdu, txVector, channel, uid),
      m_channelWidth(txVector.IsNonHtDuplicate() ? MHz_t{20} : txVector.GetChannelWidth())
{
...
void
OfdmPpdu::LSigHeader::SetRate(uint64_t rate, MHz_t channelWidth)
{
    if (channelWidth == MHz_t{5})
    {
        rate *= 4; // corresponding 20 MHz rate if 5 MHz is used
    }
    else if (channelWidth == MHz_t{10})
    {
...
uint64_t
OfdmPpdu::LSigHeader::GetRate(MHz_t channelWidth) const
{
    uint64_t rate = 0;
    switch (m_rate)
    {
