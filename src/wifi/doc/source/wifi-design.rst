...
the transmission of an ICF fails. By default, an ICF failure is treated just like any other failure:
the contention window is updated and the failure is notified to the remote station manager. However,
the advanced AP EMLSR manager provides two attributes, ``UpdateCwAfterFailedIcf`` and
``ReportFailedIcf``, that can be used, respectively, to control whether to update the contention
window and report the failure to the remote station manager in case of ICF failure. These attributes
can assume one of the following enum values:

* ``ALWAYS``: always update the contention window and report the failure to the remote station manager
* ``IF_NOT_CROSS_LINK_COLLISION``: update the contention window and report the failure to the remote
  station manager unless the ICF failed due to a cross link collision, i.e., the ICF failed because
  all the recipients of the ICF are EMLSR client(s) that are sending or have sent an ICF on another
  EMLSR link
* ``NEVER``: never update the contention window and report the failure to the remote station manager
