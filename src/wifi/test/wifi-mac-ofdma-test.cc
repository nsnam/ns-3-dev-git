...
TestMultiUserScheduler::ComputeWifiTxVector()
{
    ...
    {
        auto index{ruIndex};
        if (!IsEht(m_txVector.GetPreambleType()) && (bw == MHz_t{160}) && (ruIndex >= 3))
        {
            index = ruIndex - 2;
            primary80 = false;
        }
        if (IsEht(m_txVector.GetPreambleType()) && (bw > MHz_t{80}))
        {
      ...
      m_defaultTbPpduDuration(MilliSeconds(2))
{
    ...
    case 160:
        ...
        break;
    default:
        NS_ABORT_MSG("Unhandled channel width (" << m_channelWidth << ")");
    }
                                                 ...
                                                 {10, 127, 2047, 100} /* worse parameters */})
    {
             ...
             {WifiOfdmaScenario::HE, WifiOfdmaScenario::HE_EHT, WifiOfdmaScenario::EHT})
        {
            ...
            AddTestCase(new OfdmaAckSequenceTest(
                        ...
                        TestCase::Duration::QUICK);
            AddTestCase(
                new OfdmaAckSequenceTest({.channelWidth = MHz_t{40},
                                          .dlMuAckType = WifiAcknowledgment::DL_MU_BAR_BA_SEQUENCE,
                                          .maxAmpduSize = 10000,
                                          .txopLimit = 0,
