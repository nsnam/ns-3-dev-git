...
class WifiTxopTest : public TestCase
{
  public:
    ...
    struct Params
    {
        ...
        Time singleExchangeProtectionSurplus; //!< additional time to protect beyond end of the
  ...
  private:
    ...
    Time m_singleExchangeProtectionSurplus; ///< @copydoc Params::singleExchangeProtectionSurplus
...
WifiTxopTest::WifiTxopTest(const WifiTxopTest::Params& params)
    : TestCase("Check correct operation within TXOPs with parameters: nonHt=" +
               ...
               std::to_string(params.singleExchangeProtectionSurplus.GetMicroSeconds()) + "us"),
      m_nStations(3),
            ...
            for (const auto singleRtsPerTxop : {true, false})
            {
                for (const auto lengthBasedRtsCtsThresh : {true, false})
                {
                    for (const auto protectedIfResponded : {true, false})
                    {
                        ...
                        {
                            for (const auto& singleExchangeProtectionSurplus :
                                 {Time(), MicroSeconds(25) /* PIFS */})
                            {
                                if (!protectSingleExchange &&
                                    singleExchangeProtectionSurplus.IsStrictlyPositive())
                                {
                                    continue;
                                }
