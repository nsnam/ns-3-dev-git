...
class WifiPrimaryChannelsTest : public TestCase
{
  public:
     ...
     */
    WifiPrimaryChannelsTest(WifiStandard standard, MHz_t channelWidth, bool useDistinctBssColors);
     ...
     */
    void SendDlMuPpdu(uint8_t bss, MHz_t txChannelWidth, RuType ruType, std::size_t nRus);
    /**
     ...
     * @param nRus the number of RUs
     */
    void SendHeTbPpdu(uint8_t bss, MHz_t txChannelWidth, RuType ruType, std::size_t nRus);
    /**
     ...
     * @param nRus the number of RUs
     */
    void DoSendHeTbPpdu(uint8_t bss, MHz_t txChannelWidth, RuType ruType, std::size_t nRus);
    /**
  ...
  private:
    ...
    std::vector<std::bitset<144>> m_received; /**< whether the last packet transmitted to/from each
                                                of  the (up to 144 per BSS) stations was received */
    std::vector<std::bitset<144>>
        m_processed;                /**< whether the last packet transmitted to/from each
                                      of the (up to 144 per BSS) stations was processed */
...
WifiPrimaryChannelsTest::DoSetup()
{
    ...
    for (uint8_t bss = 0; bss < m_nBss; bss++)
    {
        channelValue.Set(WifiPhy::ChannelSegments{
            {channelNum, m_channelWidth.in_MHz(), WIFI_PHY_BAND_6GHZ, bss}});
...
WifiPrimaryChannelsTest::DoRun()
{
    ...
    const auto mc = GetModulationClassForStandard(m_standard);
    for (MHz_t txChannelWidth{20}; txChannelWidth <= m_channelWidth;
         txChannelWidth *= 2, nRounds *= 2, nApsPerRound /= 2)
    {
        nRounds = std::min<uint16_t>(nRounds, m_nBss);
        ...
        for (uint16_t round = 0; round < nRounds; round++)
        {
            for (uint32_t type = 0; type < static_cast<uint32_t>(WifiRu::GetMaxRuType(mc)); ++type)
            {
                auto ruType = static_cast<RuType>(type);
                std::size_t nRus = WifiRu::GetNRus(txChannelWidth, ruType, mc);
                std::set<uint8_t> txBss;
                if (nRus > 0)
                {
                    for (uint16_t i = 0; i < nApsPerRound; i++)
                                      ...
                                      std::size_t nRus)
{
    ...
    auto numRus{nRus};
    if ((m_standard != WIFI_STANDARD_80211ax) && (ruType == RuType::RU_26_TONE))
    {
        // for the loop, take undefined RUs for EHT into account
        numRus += (txChannelWidth / MHz_t{80});
    }
    std::size_t staIdx{0};
    for (std::size_t i = 1; i <= numRus; ++i)
    {
        ...
        if ((m_standard != WIFI_STANDARD_80211ax) && (txChannelWidth >= MHz_t{80}) &&
            (ruType == RuType::RU_26_TONE) && (index == 19))
        {
            // skip undefined RU for EHT
            continue;
        }
        const auto ru =
        ...
        NS_ABORT_IF((bss >= m_staDevices.size()) || (staIdx >= m_staDevices[bss].GetN()));
        auto staDev = DynamicCast<WifiNetDevice>(m_staDevices[bss].Get(staIdx));
        ++staIdx;
                                    ...
                                    const std::set<uint8_t>& secondary160)
{
    ...
    auto actualPrimary160 = m_channel.GetAll20MHzChannelIndicesInPrimary(MHz_t{160});
    ...
    : TestSuite("wifi-primary-channels", Type::UNIT)
{
    for (const auto standard : {WIFI_STANDARD_80211ax, WIFI_STANDARD_80211be})
    {
        ...
        AddTestCase(new WifiPrimaryChannelsTest(standard, MHz_t{40}, true),
                    TestCase::Duration::QUICK);
        AddTestCase(new WifiPrimaryChannelsTest(standard, MHz_t{40}, false),
                    TestCase::Duration::QUICK);
        AddTestCase(new WifiPrimaryChannelsTest(standard, MHz_t{80}, true),
                    TestCase::Duration::EXTENSIVE);
        AddTestCase(new WifiPrimaryChannelsTest(standard, MHz_t{80}, false),
                    TestCase::Duration::EXTENSIVE);
        AddTestCase(new WifiPrimaryChannelsTest(standard, MHz_t{160}, true),
                    TestCase::Duration::TAKES_FOREVER);
        AddTestCase(new WifiPrimaryChannelsTest(standard, MHz_t{160}, false),
                    TestCase::Duration::TAKES_FOREVER);
    }
    ...
    /*AddTestCase(new WifiPrimaryChannelsTest(WIFI_STANDARD_80211be, MHz_t{320}, true),
                TestCase::Duration::TAKES_FOREVER);
