...
WifiStaticInfraBssTest::GetWifiHelper() const
{
    ...
    // TODO wifiHelper.ConfigHeOptions("UlMuDataDisable",
    // BooleanValue(m_testVec->m_input.ulMuDataDisable));
    return wifiHelper;
}
                                             ...
                                             Ptr<StaWifiMac> clientMac)
{
    ...
    // Validate OM Control UL MU Data Disable
    // TODO auto omCtrl = apRemoteMgr->GetHeVarOmCtrl(staAddr);
    // NS_TEST_ASSERT_MSG_EQ(omCtrl.has_value(),
    //                       m_testVec->m_input.ulMuDataDisable,
    //                       "OM Control presence mismatch on link ID " << +apLinkId);
    // if (!omCtrl.has_value())
    // {
    //     return;
    // }
    // auto actualDisable = static_cast<bool>(omCtrl->m_ulMuDataDisable);
    // NS_TEST_ASSERT_MSG_EQ(actualDisable,
    //                       m_testVec->m_input.ulMuDataDisable,
    //                       "UL MU Data Disable mismatch on link ID " << +apLinkId);
}
...
WifiStaticInfraBssTest::ValidateMuScheduler(Ptr<ApWifiMac> apMac, Ptr<StaWifiMac> clientMac)
{
    ...
    /* TODO */ auto clientSize [[maybe_unused]] = clientList.size();
    std::size_t expectedSize = m_testVec.ulMuDataDisable ? 0 : 1;
    // TODO NS_TEST_ASSERT_MSG_EQ(clientSize, expectedSize, "Unexpected MU Scheduler client list
    // size");
    if (expectedSize == 0)
    {
        return;
