                ...
                Time pktInterval)
{
    ...
    {
        mac.SetType("ns3::AdhocWifiMac", "BeaconGeneration", BooleanValue(false));
        phy.Set("TxPowerStart", dBmValue(staTxPower));
        phy.Set("TxPowerEnd", dBmValue(staTxPower));
    ...
    const auto angle = (degree_t{360.0} / (nNodes - 1));
    for (uint32_t i = 0; i < (nNodes - 1); ++i)
    {
        positionAlloc->Add(Vector(1.0 + (distance * cos(i * angle.in_radian())),
                                  1.0 + (distance * sin(i * angle.in_radian())),
                                  0.0));
    }
