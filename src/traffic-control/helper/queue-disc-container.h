...
{
public:
  /// QueueDisc container iterator
  typedef std::vector<Ptr<QueueDisc> >::const_iterator Iterator;
  ...
  /**
   * \brief Get an iterator which refers to the first QueueDisc in the
   * container.
   *
   ...
   * \endcode
   *
   * \returns an iterator which refers to the first QueueDisc in the container.
   */
  Iterator Begin (void) const;

  /**
   * \brief Get an iterator which indicates past-the-last QueueDisc in the
   * container.
   *
   * QueueDiscs can be retrieved from the container in two ways.  First,
   * directly by an index into the container, and second, using an iterator.
