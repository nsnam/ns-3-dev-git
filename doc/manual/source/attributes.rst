...
.. note::
   ...
   Because these macros generate class and function definitions, it is
   difficult to document the results directly. Instead, the documentation for
   them is generated by ``utils/print-introspected-doxygen.cc``. As a
   consequence, when introducing a new ``AttributeValue`` subclass (whether
   via the ``ATTRIBUTE_HELPER_*`` macros or by hand), the new type must also
   be added to the ``attributes[]`` list in ``PrintAttributeImplementations()``
   in ``utils/print-introspected-doxygen.cc``. Otherwise the introspected Doxygen
