...
if(${XCODE})
  # Is that so hard not to break people's CI, AAPL? Why would you output the targets to a Debug/Release subfolder? Why?
...
macro(process_options)
  ...
  if(${NS3_CLANG_TIMETRACE})
    if(${CLANG})
      ...
      ExternalProject_Add(
        ...
        INSTALL_COMMAND cmake -E copy_if_different ClangBuildAnalyzer ${PROJECT_BINARY_DIR}
      )
      ...
      add_custom_target(
        timeTraceReport
        COMMAND ${PROJECT_BINARY_DIR}/ClangBuildAnalyzer --all ${PROJECT_BINARY_DIR}
                ${PROJECT_BINARY_DIR}/clangBuildAnalyzerReport.bin
        COMMAND ${PROJECT_BINARY_DIR}/ClangBuildAnalyzer --analyze ${PROJECT_BINARY_DIR}/clangBuildAnalyzerReport.bin >
                ${PROJECT_SOURCE_DIR}/ClangBuildAnalyzerReport.txt
        DEPENDS ClangBuildAnalyzer
      )
    else()
      message(FATAL_ERROR "TimeTrace is a Clang feature, but you're using a different compiler.")
    endif()
  endif()
  ...
  if(${unsupported} GREATER -1)
    message(FATAL_ERROR "You're trying to use the unsupported C++ ${CMAKE_CXX_STANDARD}.\n"
  ...
  if(${NS3_SANITIZE} AND ${NS3_SANITIZE_MEMORY})
    message(FATAL_ERROR "The memory sanitizer can't be used with other sanitizers. Disable one of them.")
  endif()
  ...
  if(${NS3_SANITIZE_MEMORY})
    if(${CLANG})
      ...
      if(NOT ($ENV{MSAN_OPTIONS} MATCHES "strict_memcmp=0"))
        message(
          WARNING "Please export MSAN_OPTIONS=strict_memcmp=0 "
  ...
  if(${NS3_LINK_TIME_OPTIMIZATION})
    ...
    if(LTO_AVAILABLE)
      ...
      message(STATUS "Link-tiem optimization (LTO) is supported.")
    else()
      message(STATUS "Link-time optimization (LTO) is not supported: ${output}.")
    endif()
  endif()
  ...
  if(${NS3_INCLUDE_WHAT_YOU_USE})
    # Before using, install iwyu, run "strings /usr/bin/iwyu | grep LLVM" to find the appropriate clang version and
    # install it. If you don't do that, it will fail. Worse than that: it will fail silently. We use the wrapper to get
    # a iwyu.log file in the ns-3-dev folder.
    ...
    set(CMAKE_CXX_INCLUDE_WHAT_YOU_USE ${PROJECT_SOURCE_DIR}/buildsupport/iwyu_wrapper.sh;${PROJECT_SOURCE_DIR})
  else()
  ...
  if(${XCODE})
    if(${NS3_STATIC} OR ${NS3_MONOLIB})
      message(
        FATAL_ERROR "Xcode doesn't play nicely with CMake object libraries,"
  ...
  if(${NS3_STATIC})
    # Warn users that they may be using shared libraries, which won't produce a standalone static library
    set(ENABLE_REALTIME FALSE)
    set(HAVE_RT FALSE)
    message(STATUS "Statically linking 3rd party libraries have not been tested.\n"
  ...
  if(NOT ${BOOST_FOUND})
    message(STATUS "Boost was not found. Continuing without it.")
  else()
...
\n"
    ...
    function(sphinx_target targetname)
      add_custom_target(
        sphinx_${targetname} COMMAND make SPHINXOPTS=-N -k html singlehtml latexpdf
  ...
  if(${INT64X64} MATCHES "DOUBLE")
    ...
    if(${SIZEOF_LONG_DOUBLE} EQUAL ${SIZEOF_DOUBLE})
      message(
        STATUS "Long double has the wrong size: LD ${SIZEOF_LONG_DOUBLE} vs D ${SIZEOF_DOUBLE}. Falling back to CAIRO."
      )
      set(INT64X64 "CAIRO")
    else()
  ...
  # Force enable ns-3 logging in debug builds and if requested for other build types
  if(${NS3_LOG} OR (${build_profile} STREQUAL "debug"))
    add_definitions(-DNS3_LOG_ENABLE)
  endif()
  # Force enable ns-3 asserts in debug builds and if requested for other build types
  ...
  if(APPLE OR WSLv1)
    ...
    message(STATUS "${PLATFORM_UNSUPPORTED_PRE} TAP and EMU ${PLATFORM_UNSUPPORTED_POST}")
  endif()
  ...
  if(${NS3_NETANIM})
    ...
    FetchContent_Declare(netanim GIT_REPOSITORY https://gitlab.com/nsnam/netanim.git GIT_TAG netanim-3.108)
...
function(copy_headers_before_building_lib libname outputdir headers visibility)
  ...
  foreach(header ${headers})
    ...
    add_custom_command(
      TARGET ${copy_headers_target} PRE_BUILD COMMAND ${CMAKE_COMMAND} -E copy_if_different
  ...
  if(${XCODE})
    add_dependencies(${lib${libname}} copy_all_headers) # Xcode doesn't play nicely with object libraries
  else()
...
macro(build_lib_example name source_files header_files libraries_to_link)
  build_lib_example_impl("src/${libname}" "${name}" "${source_files}" "${header_files}" "${libraries_to_link}")
endmacro()
  ...
  if(NOT ${missing_dependencies})
    ...
    elseif(${NS3_MONOLIB})
      ...
      target_link_libraries(
        ${name} ${LIB_AS_NEEDED_PRE} ${libraries_to_link} ${optional_visualizer_lib} ${LIB_AS_NEEDED_POST}
      )
    endif()
...
function(filter_modules modules_to_filter all_modules_list filter_in)
  ...
  # We are receiveing variable names as arguments, so we have to "dereference" them first That is why we have the
  # duplicated ${${}}
...
function(recursive_dependency module_name)
  ...
  set(contrib_cmakelist ${PROJECT_SOURCE_DIR}/contrib/${module_name}/CMakeLists.txt)
  set(contrib FALSE)
  ...
  elseif(EXISTS ${contrib_cmakelist})
    ...
    message(STATUS "The CMakeLists.txt file for module ${module_name} was not found.")
  endif()
  ...
  if(contrib)
    set(contrib_dependency_visited "${contrib_dependency_visited};${module_name}" CACHE INTERNAL "")
  ...
  foreach(match ${matches})
    if(NOT ((${match} IN_LIST dependency_visited) OR (${match} IN_LIST contrib_dependency_visited)))
