...
set yrange [0:1]\n";
  ...
  const std::string colors[14] = {"green", "blue", "red", "black", "orange", "purple", "yellow", "pink", "grey", "magenta", "brown", "turquoise", "olive", "beige"};
  for (uint32_t i = 0; i < modes.size (); i++)
    {
      plotExtra << "set style line " << +lineNumber++ << " linewidth 5 linecolor rgb \"" << colors[i] << "\" \n";
    }
